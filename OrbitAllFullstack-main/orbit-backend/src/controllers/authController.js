...
export const socialLogin = async (req, res) => {
  try {
    const { token: googleToken, provider } = req.body;

    if (provider !== 'google') {
      return res.status(400).json({ message: "Only Google is supported currently for real OAuth" });
    }
    ...
    if (!user) {
      ...
      // Link the googleId if it's not already linked
      if (!user.googleId) {
    ...
    res.status(500).json({ message: "Identity verification failed: " + error.message });
  }
};
