...
${text.substring(0, 10000)}
      ...
      const isQuota = aiError.message?.includes("429") || aiError.message?.includes("quota") || aiError.status === 429;
      
      if (isQuota) {
        console.log("🛠️ Quota exceeded - Triggering Simulated Analysis...");
        // Fallback simulated data so the user can still see the feature
        const simulatedData = {
          atsScore: 78,
          skills: ["Project Management", "React", "Data Analysis", "Communication", "Leadership"],
          experience: "3 years (Simulated)",
          suggestions: ["Wait 1 minute for full AI insights", "Optimize keywords for specific roles"],
          trackMatches: [
        ...
        return res.status(200).json({
          message: "Simulated Analysis (AI Quota Full)",
          ...simulatedData,
          roadmapProgress: 45,
          jobsMatched: 12,
          isSimulated: true
